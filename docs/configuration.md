...
| `PROXY_AGENT_MANAGER_TLS_BUNDLE` | _(none)_ | Single PEM file holding the internal-client cert, its key, and the internal CA cert (the consolidated `internal-client` Docker secret). When set it supersedes the three `PROXY_AGENT_MANAGER_TLS_*` vars below. Either this **or** all three of them are required when `PROXY_AGENT_MANAGER_URL` is `wss://`. |
| `PROXY_AGENT_MANAGER_TLS_CERT` | _(none)_ | Client certificate the proxy presents to the agent-manager (mutual TLS). Fallback for older bootstraps when `PROXY_AGENT_MANAGER_TLS_BUNDLE` is unset. |
